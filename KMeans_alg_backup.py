...
"""
def mean_of_points(centers_n_pts, cols, group, data_point):
    new_centroids = []
    points_clusters = {}
    data_pt = {}
...
data_pt, data_pt_lst = mean_of_points(centers_n_pts, cols, group, data_point)
...
def update_cluster_centroids(data_point, new_centroids_lst):
    ...
    try:
        iter = int(input("\nPlease enter number of iteration for KMeans alg.: "))
    except ValueError or KeyboardInterrupt:
        iter = 50
    ...
    return new_centroids_lst
############################################################################################################
############################################################################################################
############################################################################################################
############################################################################################################

new_centroids_lst = update_cluster_centroids(data_point, new_centroids_lst)
...
def plot(df, new_centroids_lst):
    ...
    if plotting==1:
        ...
        axes[0].scatter(df[cols_names[0]], df[cols_names[1]], alpha=1)
        axes[1].scatter(df[cols_names[0]], df[cols_names[1]], c=eacleadian_dist, alpha=1)
        axes[1].scatter(new_centroids_lst[:, 0], new_centroids_lst[:,1], marker="*", s=100, c='g')
